...
with mlflow.start_run(run_name=f"Model Training (Churn prediction)-{timestamp}") as parent_run:
    ...
    # Base model là một pipeline kết hợp PCA và Naive Bayes
    base_model_pca_nb = ImPipeline(steps=[
        ("pca", PCA(random_state=42)),
        ("classifier", GaussianNB())
    ])

    # Parameter grid cho pipeline
    # Cú pháp: 'tên_bước__tên_tham_số'
    param_grid_pca_nb = {
        'clf__pca__n_components': range(10, 20,2),
        'clf__classifier__var_smoothing': np.logspace(0, -9, num=10)
    }

    # Cấu hình HalvingGridSearchCV cho pipeline
    halving_search_pca_nb = HalvingGridSearchCV(
        estimator=base_model_pca_nb,
        param_grid=param_grid_pca_nb,
        scoring="f1_weighted",
        cv=5,
        factor=3,
        verbose=1,
    ...
    # Chạy tuner cho pipeline
    with AdvancedModelTuner(
        search_strategy=halving_search_pca_nb,
        preprocessor=preprocessor,
        sampler=sampler,
        experiment_name=EXPERIMENT_NAME,
        pip_requirements=INFERENCE_PIP_REQUIREMENTS
        ...
        print("\n--- Bắt đầu Huấn luyện với HalvingGridSearchCV + PCA + Naive Bayes ---")
        tuner.fit(X_train, y_train)
    ...
    # Chia tập train thành train_main và validation để dùng early stopping
    ...
    base_model_xgb = XGBClassifier(
        ...
        n_estimators=1000, # Đặt số estimators lớn để early stopping quyết định
        early_stopping_rounds=20
    )

    # === Chuyển đổi từ param_grid sang search_spaces cho BayesSearchCV ===
    # === search_spaces được bổ sung nhiều tham số điều chuẩn ===
    search_spaces_xgb = {
        # Các tham số gốc
        "max_depth": Integer(3, 6),  # Giữ nguyên hoặc giảm nhẹ dải tìm kiếm
        "learning_rate": Real(0.01, 0.2, prior='log-uniform'), # Giảm nhẹ cận trên
        "subsample": Real(0.6, 0.9, prior='uniform'), 
        "colsample_bytree": Real(0.6, 0.9, prior='uniform'),

        # === Bổ sung các tham số điều chuẩn mạnh mẽ ===

        # gamma: Yêu cầu việc giảm loss phải đủ lớn thì mới phân nhánh.
        # Giá trị càng lớn, mô hình càng bảo thủ (ít phức tạp hơn).
        "gamma": Real(0, 5, prior='uniform'),
        
        # min_child_weight: Lượng trọng số (samples) tối thiểu cần có trong một nhánh con.
        # Giúp ngăn cây học những quy luật chỉ đúng trên một vài mẫu.
        "min_child_weight": Integer(1, 10),
        
        # reg_alpha (L1 Regularization): Phạt trên trọng số tuyệt đối.
        # Có thể khiến một số trọng số feature về 0 (chọn lọc feature).
        "reg_alpha": Real(0.01, 10.0, prior='log-uniform'),
        
        # reg_lambda (L2 Regularization): Phạt trên trọng số bình phương.
        # Giúp làm "mượt" các trọng số, tránh phụ thuộc quá nhiều vào một vài feature.
        "reg_lambda": Real(0.01, 10.0, prior='log-uniform')
    }

    # === Thay HalvingGridSearchCV bằng BayesSearchCV ===
    bayes_search_strategy = BayesSearchCV(
        estimator=base_model_xgb,
        search_spaces=search_spaces_xgb, # Dùng search_spaces thay vì param_grid
        n_iter=25,                      # Số lần thử kết hợp tham số (quan trọng)
        scoring="f1_weighted",
        cv=5,
        verbose=1,
        random_state=42,
    ...
    # Giữ nguyên cấu trúc AdvancedModelTuner
    with AdvancedModelTuner(
        search_strategy=bayes_search_strategy, 
        preprocessor=preprocessor,
        sampler=sampler,
        experiment_name=EXPERIMENT_NAME,
        ...
        print("--- Bắt đầu Huấn luyện với BayesSearchCV + XGBoost ---")
        # Phương thức fit_discrete vẫn hoạt động tốt vì BayesSearchCV cũng truyền 
        # eval_set vào hàm fit của XGBoost để dùng early stopping.
    ...
    # Base model cho Logistic Regression
    ...
    # Parameter grid cho Logistic Regression
    param_grid_lr = {
        'penalty': ['l1', 'l2'],
        "C": [0.01,0.05,0.1, 0.5,1,2, 5, 10,15,20,50,70,100],
        'solver': ['saga'] # Solver 'saga' hỗ trợ cả L1 và L2
    }
    
    # Cấu hình HalvingGridSearchCV cho Logistic Regression
    halving_search_lr = HalvingGridSearchCV(
    ...
    # Chạy tuner cho Logistic Regression
    with AdvancedModelTuner(
        search_strategy=halving_search_lr,
        preprocessor=preprocessor,
        sampler=sampler,
        experiment_name=EXPERIMENT_NAME,
        pip_requirements=INFERENCE_PIP_REQUIREMENTS
        ...
        print("\n--- Bắt đầu Huấn luyện với HalvingGridSearchCV + Logistic Regression ---")
        # Sử dụng lại bộ dữ liệu đã split
        tuner.fit(X_train, y_train)
    ...
    base_model_svc = SVC(
        random_state=42,
        probability=True # Cần thiết để tính toán một số metrics và cho API chuẩn
    )
    
    # Chạy tuner cho SVC
    with AdvancedModelTuner(
        model=base_model_svc,
        preprocessor=preprocessor,
        sampler=sampler,
        experiment_name=EXPERIMENT_NAME,
        pip_requirements=INFERENCE_PIP_REQUIREMENTS
        ...
        print("\n--- Bắt đầu Huấn luyện với HalvingGridSearchCV + SVC ---")
        tuner.fit(X_train, y_train)
    ...
    #                             DECISION TREE
    # ==============================================================================
    base_model_et = DecisionTreeClassifier(
    random_state=42
    )

    # --- Chuyển từ param_distributions sang search_spaces ---
    search_spaces_et = {
        'sampler__k_neighbors': Integer(1, 5),
        'criterion': Categorical(['gini', 'entropy']),
        'max_features': Categorical(['sqrt', 'log2', None]),
        'min_samples_split': Integer(2, 35),
        'min_samples_leaf': Integer(2, 35),
        'max_depth': Integer(3, 20),
    }

   
    bayes_search_et = BayesSearchCV(
        estimator=base_model_et,
        search_spaces=search_spaces_et, # <-- Dùng search_spaces
        n_iter=50,                      # <-- Số lần thử (ngân sách tìm kiếm)
        scoring="f1_weighted",
        cv=5,
        verbose=1,
        random_state=42
    ...
    # Chạy tuner 
    with AdvancedModelTuner(
        search_strategy=bayes_search_et, #
        preprocessor=preprocessor,
        sampler=sampler,
        experiment_name=EXPERIMENT_NAME,
        pip_requirements=INFERENCE_PIP_REQUIREMENTS
        ...
        print("\n--- Bắt đầu Huấn luyện với BayesSearchCV + DecisionTreeClassifier ---")
        tuner.fit(X_train, y_train)
    ...
    # ==============================================================================
    base_model_et = ExtraTreesClassifier(
    random_state=42,
    bootstrap=True, # <-- Đặt tham số cố định ở đây
    n_jobs=-1
)
    ...
    # --- Thay HalvingRandomSearchCV bằng BayesSearchCV ---
    bayes_search_et = BayesSearchCV(
        estimator=base_model_et,
    ...
    # Chạy tuner cho ExtraTreesClassifier
    with AdvancedModelTuner(
        search_strategy=bayes_search_et, 
        preprocessor=preprocessor,
        sampler=sampler,
        ...
        print("\n--- Bắt đầu Huấn luyện với BayesSearchCV + ExtraTreesClassifier ---")
        tuner.fit(X_train, y_train)
